<template>
    <div>
        <Header />
        <v-container
            :class="{'pa-16':$vuetify.breakpoint.mdAndUp}"
        >
            <v-breadcrumbs :items="breadcrumbsItems"></v-breadcrumbs>
            <h2 class="display-1 pl-5 pb-2">Detalji ponude</h2>
            <v-container class="pa-5">
            <v-row class="my-2">
                <!-- LIJEVO, A NA MOBILE IZNAD -->
                <v-col>

                    <v-row align="center">
                        <v-col cols="6">
                        <v-img
                            src="data:image/png;base64,"
                            alt="Company logo"
                            width="92"
                            contain
                        ></v-img>
                        </v-col>
                        <v-col
                            class="text-center"
                            cols="6"
                            >
                            <v-chip
                            class="mr-4"
                            color="orange"
                            >
                            U planu
                            </v-chip>
                        </v-col>
                    </v-row>

                    <v-row align="start">
                        <v-col cols="6">
                        <p>Široki Brijeg - Genova</p>
                        <p>Planirano vrijeme polaska: 14.1.2022.</p>
                        <p>Planirano vrijeme dolaska: 15.1.2022.</p>
                        </v-col>
                        <v-col
                            class="text-left"
                            cols="6"
                            >
                        <p>Raspoloživo: 7 m<sup>3</sup>, 10 t</p>
                        <p>Dimenzije: 190 x 190 x 200 cm</p>
                        </v-col>
                    </v-row>

                    <v-row align="start">
                        <v-col cols="6">
                        <p>
                            <v-icon>mdi-account-box</v-icon> Odgovorna osoba: Mijo Mijić
                        </p>
                        <p>
                            <v-icon>mdi-phone</v-icon> Kontakt tel.: +387 63 123 456
                        </p>
                        </v-col>
                        <v-col
                            class="text-left"
                            cols="6"
                            >
                        <p>
                            <v-icon>mdi-clock-time-seven</v-icon> Radno vrijeme: 07:00 - 15:30
                        </p>

                        </v-col>
                    </v-row>
                    <v-row align="start">
                        <v-col>
                            <p class="subtitle-2">Dodatne informacije</p>
                            <p> Lorem ipsum dolor sit, amet consectetur adipisicing elit. Facilis, magni ullam atque amet modi iusto illum incidunt excepturi debitis, voluptas porro sunt, numquam possimus nisi. </p> 
                        </v-col>
                        
                    </v-row>
                    
                </v-col>
                <!-- LIJEVO, A NA MOBILE IZNAD KRAJ-->

                <!-- DESNO, A NA MOBILE ISPOD -->
                <v-col class="d-flex justify-center">
                    <v-card max-width="320px" class="pa-3">
                        <v-card-text>
                            <p>Ime tvrtke: Company AG</p>
                            <p>Objavljeno: 6.1.2021.</p>
                            <p>ID ponude: 455</p>
                        </v-card-text>
                        <v-card-actions class="d-flex flex-column">

                            <!--************************-->
                            <!--Forma za slanje zahtjeva-->
                            <!--************************-->
                            <v-dialog
                                v-model="dialog"
                                persistent
                                max-width="600px"
                            >
                                <template v-slot:activator="{ on, attrs }">

                                <v-btn 
                                    class="my-2 secondary justify-start"
                                    block
                                    v-bind="attrs"
                                    v-on="on"
                                > 
                                    <v-icon class="mr-1">mdi-offer</v-icon> Pošalji zahtjev
                                </v-btn>
                                </template>
                                <v-card>
                                <v-card-title>
                                    <span class="text-h5">Novi zahtjev za prijevoz tereta</span>
                                </v-card-title>
                                <v-card-text>
                                    <v-container>
                                    <v-row>
                                        <v-col
                                        cols="12"
                                        sm="6"
                                        md="6"
                                        >
                                        <v-text-field
                                            label="Mjesto preuzimanja*"
                                            required
                                        ></v-text-field>
                                        </v-col>
                                        <v-col
                                        cols="12"
                                        sm="6"
                                        md="6"
                                        >
                                        <v-text-field
                                            label="Adresa preuzimanja*"
                                            required
                                        ></v-text-field>
                                        </v-col>

                                        <v-col
                                        cols="12"
                                        sm="6"
                                        md="6"
                                        >
                                        <v-text-field
                                            label="Mjesto dostave*"
                                            required
                                        ></v-text-field>
                                        </v-col>
                                        
                                        <v-col
                                        cols="12"
                                        sm="6"
                                        md="6"
                                        >
                                        <v-text-field
                                            label="Adresa dostave*"
                                            required
                                        ></v-text-field>
                                        </v-col>
                                      
                                        <v-col
                                        cols="12"
                                        sm="6"
                                        >
                                        <v-autocomplete
                                            :items="['Utovar', 'Stavka2']"
                                            label="Dodatne usluge"
                                            multiple
                                        ></v-autocomplete>
                                        </v-col>
                                    </v-row>
                                    </v-container>
                                    <small>*obavezna polja</small>
                                </v-card-text>
                                <v-card-actions>
                                    <v-spacer></v-spacer>
                                    <v-btn
                                    color="blue darken-1"
                                    text
                                    @click="dialog = false"
                                    >
                                    Odustani
                                    </v-btn>
                                    <v-btn
                                    color="blue darken-1"
                                    text
                                    @click="dialog = false"
                                    >
                                    Pošalji
                                    </v-btn>
                                </v-card-actions>
                                </v-card>
                            </v-dialog>

                            <!--************************-->
                            <!--Forma za slanje zahtjeva kraj-->
                            <!--************************-->
                            <v-btn class="my-2 primary justify-start" block> <v-icon class="mr-1">mdi-share</v-icon> Podijeli</v-btn>
                            <v-btn class="my-2 error justify-start" block> <v-icon class="mr-1">mdi-alert-octagon</v-icon> Prijavi</v-btn>
                        </v-card-actions>
                    </v-card>
                </v-col>
                <!-- DESNO, A NA MOBILE ISPOD KRAJ-->
            </v-row>

            <v-row class="px-8 my-7">
                <v-col>
                    <v-img 
                        src="https://t4.ftcdn.net/jpg/02/54/26/61/360_F_254266198_5NbM7Cv3s93lYdQscnUKmizmAUKDY3YU.jpg"
                        contain
                        max-height="500px"
                    >
                    </v-img>  
                </v-col>
                <v-col></v-col>

            </v-row>

            </v-container>


           

        </v-container>
        <Footer />
    </div>
</template>

<script>
import Header from '../../components/layout/TheHeader.vue'
import Footer from '../../components/layout/TheFooter.vue'
export default {
    name: "Offer",
    components: { Header, Footer },
    data: () => ({
      breadcrumbsItems: [
        {
          text: 'Početna',
          disabled: false,
          href: '/',
        },
        {
          text: 'Ponude',
          disabled: false,
          href: '/ponude',
        },
        {
          text: 'Ponuda ID 455',
          disabled: true,
          href: '',
        },
      ],
    }),
  
    methods: {
      
    }
}
</script>

<style scoped>
.facebook:hover{
    color: #4267B2;
}
.whatsapp:hover{
    color: #25D366;
}
.print:hover{
    color: black;
}.email:hover{
    color: #CA2621;
}
</style>